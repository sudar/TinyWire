/*
  TinyWireMaster.h - a wrapper(+) class for TWI/I2C Master library for the ATtiny on Arduino
  ...
  I added Atmel's original Device dependant defines section back into usiTwiMaster.h
 ...
 USAGE is modeled after the standard Wire library . . .
  ...
  To Send:
	...
	someByte = TinyWireM.endTransmission(){          // actually send the bytes in the buffer
	                                                 // returns (optional) 0 = sucess or see usiTwiMaster.h for error codes
  To Receive:
	someByte = TinyWireM.requestFrom(uint8_t slaveAddr, uint8_t numBytes){      // reads 'numBytes' from slave's address
	                                                 // (usage optional) returns 0= success or see usiTwiMaster.h for error codes
...
#ifndef TinyWireMaster.h
#define TinyWireMaster.h
